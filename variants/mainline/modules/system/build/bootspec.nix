    ...
    v1 = rec {
      ...
      json =
        ...
        (builtins.toJSON
          ...
          (cfg.extensions //
          {
            "org.expidus.bootspec.v1" = {
              system = config.boot.kernelPackages.stdenv.hostPlatform.system;
              kernel = "${config.boot.kernelPackages.kernel}/${config.system.boot.loader.kernelFile}";
              kernelParams = config.boot.kernelParams;
              label = "${config.system.expidus.distroName} ${config.system.expidus.codeName} ${config.system.expidus.label} (Linux ${config.boot.kernelPackages.kernel.modDirVersion})";
      ...
      generator =
        let
          ...
          toplevelInjector = lib.escapeShellArgs [
            ...
            ''
              ."org.expidus.bootspec.v1".toplevel = $toplevel |
              ."org.expidus.bootspec.v1".init = $init
            ''
            "--sort-keys"
          ...
          specialisationInjector =
            ...
            lib.escapeShellArgs [
              "${pkgs.buildPackages.jq}/bin/jq"
              "--sort-keys"
              ''."org.expidus.specialisation.v1" = ($ARGS.named | map_values(. | first))''
