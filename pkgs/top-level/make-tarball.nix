...
, nix ? pkgs.nix
, lib-tests ? import "${pkgs.lib.expidus.channels.nixpkgs}/lib/tests/release.nix" { inherit pkgs; }
}:
pkgs.releaseTools.sourceTarball {
  name = "expidus-packages-tarball";
  src = nixpkgs;
  ...
  configurePhase = ''
    eval "$preConfigure"
    releaseName=expidus-packages-$VERSION$VERSION_SUFFIX
  ...
  nixpkgs-basic-release-checks = import "${pkgs.lib.expidus.channels.nixpkgs}/pkgs/top-level/nixpkgs-basic-release-checks.nix"
  ...
  checkPhase = ''
    ...
    nix-env -f . -I nixpkgs=$src -qa --meta --json --arg config 'import ${pkgs.lib.expidus.channels.nixpkgs + "/packages-config.nix"}' "''${opts[@]}" >> tmp
    echo -n '}' >> tmp
