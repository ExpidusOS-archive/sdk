  ...
  amazonImage = forMatchingSystems [ "x86_64-linux" "aarch64-linux" ] (system:
    ...
    hydraJob ((import lib/eval-config.nix {
      ...
      modules =
        [ configuration
          versionModule
          ((import ../lib/nixpkgs.nix) + "/maintainers/scripts/ec2/amazon-image.nix")
        ];
  ...
  amazonImageZfs = forMatchingSystems [ "x86_64-linux" "aarch64-linux" ] (system:
    ...
    hydraJob ((import lib/eval-config.nix {
      ...
      modules =
        [ configuration
          versionModule
          ((import ../lib/nixpkgs.nix) + "/maintainers/scripts/ec2/amazon-image-zfs.nix")
        ];
    }).config.system.build.amazonImage)
  );
  ...
  lxdImage = forMatchingSystems [ "x86_64-linux" "aarch64-linux" ] (system:
    ...
    hydraJob ((import lib/eval-config.nix {
      ...
      modules =
        [ configuration
          versionModule
          ((import ../lib/nixpkgs.nix) + "/maintainers/scripts/lxd/lxd-image.nix")
        ];
    }).config.system.build.tarball)
