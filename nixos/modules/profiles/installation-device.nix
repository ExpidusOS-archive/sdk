...

{
  ...
  config = {
    ...
    services.getty.helpLine = ''
      The "expidus" and "root" accounts have empty passwords.

      An ssh daemon is running. You then must set a password
      for either "root" or "expidus" with `passwd` or add an ssh key
      to /home/expidus/.ssh/authorized_keys be able to login.

      If you need a wireless connection, type
      `sudo systemctl start wpa_supplicant` and configure a
      network using `wpa_cli`. See the ExpidusOS manual for details.
