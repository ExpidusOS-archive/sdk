...
my $start_list_file = "/run/expidus/start-list";
my $restart_list_file = "/run/expidus/restart-list";
my $reload_list_file = "/run/expidus/reload-list";
...
my $restart_by_activation_file = "/run/expidus/activation-restart-list";
my $reload_by_activation_file = "/run/expidus/activation-reload-list";
my $dry_restart_by_activation_file = "/run/expidus/dry-activation-restart-list";
my $dry_reload_by_activation_file = "/run/expidus/dry-activation-reload-list";
...
# This is a NixOS installation if it has /etc/EXPIDUS or a proper
# /etc/os-release.
if (!-f "/etc/EXPIDUS" && (read_file("/etc/os-release", err_mode => "quiet") // "") !~ /^ID="?expidusos"?/msx) {
    die("This is not an ExpidusOS installation!\n");
}

make_path("/run/expidus", { mode => oct(755) });
openlog("expidus", "", LOG_USER);
