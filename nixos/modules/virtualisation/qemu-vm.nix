...
with lib;
let
  qemu-common = import ("${lib.expidus.channels.nixpkgs}/nixos/lib/qemu-common.nix") { inherit lib pkgs; };
      ...
      ${lib.optionalString (cfg.useNixStoreImage)
        (if cfg.writableStore
          then ''
            ...
            ${qemu}/bin/qemu-img create -f qcow2 -F qcow2 -b ${storeImage}/expidus.qcow2 "$TMPDIR"/store.img
          ''
          else ''
            (
  ...
  bootDisk =
    pkgs.vmTools.runInLinuxVM (
      pkgs.runCommand "expidus-boot-disk"
        { preVM =
            ''
              mkdir $out
  ...
  storeImage = import ("${lib.expidus.channels.nixpkgs}/nixos/lib/make-disk-image.nix") {
...
{
  imports = [
    ("${lib.expidus.channels.nixpkgs}/nixos/modules/profiles/qemu-guest.nix")
  ...
  config = {
    ...
    system.build.vm = cfg.host.pkgs.runCommand "expidus-vm" {
