...
let
  rootfsImage = pkgs.callPackage "${lib.expidus.channels.nixpkgs}/nixos/lib/make-ext4-fs.nix" ({
    inherit (config.sdImage) storePaths;
    compressImage = config.sdImage.compressImage;
    populateImageCommands = config.sdImage.populateRootCommands;
    volumeLabel = "EXPIDUS_SD";
...
in
{
  ...
  options.sdImage = {
    imageName = mkOption {
      default = "${config.sdImage.imageBaseName}-${config.system.expidus.label}-${pkgs.stdenv.hostPlatform.system}.img";
  ...
  config = {
    ...
    boot.postBootCommands = lib.mkIf config.sdImage.expandOnBoot ''
      ...
      if [ -f /nix-path-registration ]; then
        ...
        # nixos-rebuild also requires a "system" profile and an /etc/EXPIDUS tag.
        touch /etc/EXPIDUS
